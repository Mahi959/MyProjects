...
//        if (retryAnalyzer != null && retryAnalyzer.getRetryCount() < Integer.parseInt(System.getProperty("failedRetryAttempts"))) {
...
//        test.log(Status.SKIP, "Test case failed after retries: " + result.getName());
//        test.log(Status.SKIP, "Exception: " + result.getThrowable());
//    }

    @Override
    public void onTestSuccess(ITestResult result) {
        RetryAnalyzer retryAnalyzer = (RetryAnalyzer) result.getMethod().getRetryAnalyzer(result);
        if (retryAnalyzer != null && retryAnalyzer.getRetryCount() > 0) {
            System.out.println("Test passed after retries: " + result.getName());
        }
        test = extent.createTest(result.getName());
    ...
    public void onTestFailure(ITestResult result) {
        ...
        try {
            screenshotPath = AppUtil.getScreenshot(DriverManager.getDriver(), result.getName());
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        test.addScreenCaptureFromPath(screenshotPath, "Screenshot of the failure"); // Attach screenshot to Extent Report
